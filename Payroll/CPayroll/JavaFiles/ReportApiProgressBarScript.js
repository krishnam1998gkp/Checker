...
var RptBuilderProcessSummary = function (thr) {
  var urlPost =
    "/" +
    _AppDomain +
    "/CPayroll/ScriptServices/ReportApiProgressBarService.asmx/GetCustomProcessStatus";
  ...
  var successFunction = function (data) {
    ...
    if (statusOfProcess.toUpperCase() == "ERROR") {
      ...
      if (
        parseInt(TotalRecords) == 0 &&
        ErrMsg.toLowerCase() == "no record found!"
      ) {
      ...
      if (countProcessedSalary > 0 || countProcessedSalary >= 0) {
        var returnPct = UpdateProgressBar(
          countTotalToProcess,
          countProcessedSalary,
          TotalRecords,
          ExcelRowsProcessed,
          ExcelRowsDividedBy
        );
        if (returnPct < 50) {
          if (TimeSpan > 0 && TimeSpan % 3 == 0) {
            $("#CommonProgressBarBody").html(
              "Processing speed may vary during this process...."
            );
          } else if (TimeSpan > 0 && TimeSpan % 4 == 0) {
            $("#CommonProgressBarBody").html(
              "Process is still in progress. Data may take more time to complete..."
            );
          } else if (TimeSpan > 0 && TimeSpan % 5 == 0) {
            $("#CommonProgressBarBody").html(
              "Please ensure a stable connection to avoid interruptions..."
            );
          } else if (TimeSpan > 0 && TimeSpan % 6 == 0) {
            $("#CommonProgressBarBody").html(
              "Loading a large dataset. Progress might appear slow but will continue until complete."
            );
          } else if (TimeSpan > 0 && TimeSpan % 2 == 0) {
            $("#CommonProgressBarBody").html(
              "Please do not close this window while the data is being prepared..."
            );
          } else {
            $("#CommonProgressBarBody").html(
              "Keep this page open until the process is complete to avoid issues..."
            );
          }
          ...
          if (returnPct > 50 && returnPct <= 60)
            $("#totalProcessed").html(
              returnPct +
                "% Generating excel file, please wait for the moment..."
            );
            ...
            $("#totalProcessed").html(
              returnPct +
                "% Please do not close this window while the excel is being generated..."
            );
          else if (returnPct > 80 && returnPct <= 88)
            $("#totalProcessed").html(
              returnPct + "% Wrapping things up, Thankyou for being with us..."
            );
          else if (returnPct > 88 && returnPct <= 98)
            $("#totalProcessed").html(
              returnPct + "% Almost there, Please hold for a moment..."
            );
            ...
            $("#totalProcessed").html(
              "99% Excel file is ready. Zipping and Downloading..."
            );
            //Show dynamic messages
            if (TimeSpan > 0 && TimeSpan % 2 == 0) {
              $("#CommonProgressBarBody").html(
                "Please do not close this window, file is zipping and downloading..."
              );
            } else {
              $("#CommonProgressBarBody").html(
                "Due to the large file size, zipping and downloading may take some time..."
              );
            }
            $("#CommonProgressBarBody").show();
          } else $("#totalProcessed").html(returnPct + "% " + step_message);
        }
      }
  ...
  function UpdateProgressBar(
    ...
    if (rowsprocessed > 0 && totalrecords > 0 && divideby > 0) {
      pct =
        pct + Math.round(Math.round((rowsprocessed / totalrecords) * 100) / 2);
    }
    //pct = pct + '%';
...
var ExcelProcessBarLockedSummary = function () {
  ...
  var urlPost =
    "/" +
    _AppDomain +
    "/CPayroll/ScriptServices/ReportApiProgressBarService.asmx/CheckExcelProcessbarAlready";
  var dataToPost = "{'_process':'" + _processType + "'}";
  var successFunction = function (data) {
    var responseObj = "";
    try {
    ...
    if (_ServiceStatus == "1" && parseInt(_IsAbleToStart) == "0") {
      let strmsg =
        "" +
        _RptName +
        " is already processing. Please wait till the completion.";
...
function CloseExcelLockedStatusInterval() {
  try {
    ...
    console.log("Error in stopProgressBar() i.e. " + e);
  }
}
function initiateProgressModal(title, message, timeIntervalInExecution = 2000) {
  $("#CommonProgressBarStatusWrapper .progressbar-outer").removeClass(
    "border-error"
  );
...
function InitiateYTDSalRegExcelProcess(
  title,
  message,
  timeIntervalInExecution = 2000
) {
  $("#CommonProgressBarStatusWrapper .progressbar-outer").removeClass(
    "border-error"
  );
  ...
  var successFunction = function (data) {
    ...
    //Process Completed
      ...
      if (report_service === "Y") {
        window.open(
          report_service_url +
            "/api/service/payrollReportFramework/download-xlsx-zip?fileName=" +
    ...
    //Process in working
      ...
      $("#statusWrapper").show();
      if (
        countProcessedSalary > 0 ||
        (countProcessedSalary >= 0 && _processType == "YTDSALREG")
      ) {
        var returnPct = UpdateProgressBar(
          countTotalToProcess,
          countProcessedSalary,
...
function InitiateDynamicRegisterExcelProcess(
  title,
  message,
  timeIntervalInExecution = 2000
) {
...
function InitiateBonusCustomReportExcelProcess(
  title,
  message,
  timeIntervalInExecution = 2000
) {
...
var BonusCustomReportExcelProcessSummary = function (thr) {
  ...
  var successFunction = function (data) {
    ...
    //Process in working
      ...
      if (
        countProcessedSalary > 0 ||
        (countProcessedSalary >= 0 && _processType == "BONUSREPORTCUSTOM")
      ) {
        var returnPct = UpdateProgressBar(
          countTotalToProcess,
          countProcessedSalary,
